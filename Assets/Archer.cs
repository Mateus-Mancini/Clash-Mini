...
public class Archer : MonoBehaviour, IPiece
{
    float _interval = 1f;

    public int PieceType { get; } = 1;
    [field: SerializeField] public int PieceTeam { get; set; } = 0;
    [field: SerializeField] public float PieceHealth { get; set; } = 18f;
    ...
    void Update()
    {
        ...
        {
            string result = GetBestAction(position, BoardManager.GetEnemies(PieceTeam), BoardManager.GetWalls());
            Debug.Log(result);
    ...
    string GetBestAction(Vector2Int position, List<Vector2Int> enemyPositions, List<Vector2Int> wallPositions)
    {
        ...
        foreach (var (dx, dy) in attackDirections)
        {
            ...
            while (nx >= 0 && nx < 8 && ny >= 0 && ny < 8)
            {
                nx += dx;
                ny += dy;
                if (walls.Contains(new Vector2Int(nx, ny))) break; // Wall blocks attack
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (var (dx, dy) in attackDirections)
            {
                ...
                while (nx >= 0 && nx < 8 && ny >= 0 && ny < 8)
                {
                    nx += dx;
                    ny += dy;
                    if (walls.Contains(new Vector2Int(nx, ny))) break;
            ...
            foreach (var (dx, dy) in moveDirections)
            {
                ...
                if (nx >= 0 && nx < 8 && ny >= 0 && ny < 8 && !walls.Contains(new Vector2Int(nx, ny)) && !enemies.Contains(new Vector2Int(nx, ny)))
                {
